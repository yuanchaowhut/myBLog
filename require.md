...
- 主动加载模块的一个<span style='background-color:yellow'>特点</span>就是在context.nextTick中会生成一个有内部名称(internal name: '_@r' + number) 的模块（<span style='background-color:yellow'>内部模块</span>），其作用是啥呢？
